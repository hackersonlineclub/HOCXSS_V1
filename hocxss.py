...
def submit_form(form_details, url, value,TOR,cookie):
    session = get_session(TOR,cookie)
...
def scan_xss(url,script,TOR,cookie):
	...
	for form in forms:
		...
		for js_script in script:
			content = submit_form(form_details, url, js_script,TOR,cookie).content.decode()
...
def crawl(url,depth,TOR,payload,cookie):
	urls=links_to_page(url,TOR,cookie)
	for url in urls:
		p=Process(target=scan_xss, args=(url,payload,TOR,cookie)) 	#scan_xss(url,payload,TOR)
		p.start()
		p.join()
		if depth != 0:
			crawl(url,depth-1,TOR,payload,cookie)
		else:
			break	
...
def XSSENTRURLPAYLOAD(TOR, OPTION):
	...
	if (TOR == False):
		try:
			...
			XSSENTRURLPAYLOAD(TOR, OPTION)
	if(OPTION == 1):
		scan_xss(weburl,payload,TOR,cookies)
	else:
		scan_xss(weburl,payload,TOR,cookies)
		crawl(weburl,depth,TOR,payload,cookies)
#----------------------------------- XSS01 function -----------------------------------
def XSS01():
	TOR = False
	os. system('clear')
