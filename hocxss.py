...
def intro():
    ...
    Team Hackersonlineclub is not hold responsible for any 
    damage or halm that is done. Using this tool to hack other 
    peoples website can lead to some serious trouble.
    ''' + '\n' + '\n' +N
	for c in intro1:
		print(c,end='')
		sys.stdout.flush()
...
def submit_form(form_details, url, value,TOR):
    ...
    if form_details["method"] == "post":
        ...
        ya = str(x.status_code)
        if(int(ya)>=400):
               PL.inforR(' WAF Detected status code  :-'+ya)
        else:
               PL.inforG(' No WAF Detected status code  :-'+ya)
